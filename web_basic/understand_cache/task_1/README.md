...
- [キャッシュに関する調査内容](#%E3%82%AD%E3%83%A3%E3%83%83%E3%82%B7%E3%83%A5%E3%81%AB%E9%96%A2%E3%81%99%E3%82%8B%E8%AA%BF%E6%9F%BB%E5%86%85%E5%AE%B9)
  ...
  - [キャッシュすべき、すべきではないもの](#%E3%82%AD%E3%83%A3%E3%83%83%E3%82%B7%E3%83%A5%E3%81%99%E3%81%B9%E3%81%8D%E3%81%99%E3%81%B9%E3%81%8D%E3%81%A7%E3%81%AF%E3%81%AA%E3%81%84%E3%82%82%E3%81%AE)
    - [Fastlyによるコンテンツ定義](#fastly%E3%81%AB%E3%82%88%E3%82%8B%E3%82%B3%E3%83%B3%E3%83%86%E3%83%B3%E3%83%84%E5%AE%9A%E7%BE%A9)
  - [意見](#%E6%84%8F%E8%A6%8B)
...
- 重要なこと
  - キャッシュサイズは意外と小さい
    - キャッシュは消える可能性がある
  - キャッシュ環境はユーザーの端末に左右されるので、あまり信頼しない
...
|コンテンツの種類|概要|例|キャッシュについて|
|------------|---------------------|-----------------|-----------------|
|静的コンテンツ|頻繁に変更されないコンテンツ|・画像<br>・CSS<br>・JavaScript|・キャッシュ可能<br>・CDNでキャッシュすることが多い|
|動的コンテンツ|動的で完全にキャッシュできないコンテンツ<br>コンテンツは毎回一意であり、2度も同じになることはない|・多くの場合のAJAX呼び出し<br>・パーソナライズされたコンテンツ(ユーザのログインやクレジットカードのトランザクションなど)|・CDNではキャッシュ不可能だが、その代わりにCDNはオリジンサーバから可能な限り迅速に効率的に配信するべき(DSAの仕組みを利用する)|
|イベント駆動型コンテンツ|未知の期間、静的なコンテンツ<br>変更押される場合とされない場合があり、いつ変更されるか事前にはわからない|・Wikiページ(数週間変更なしでも問題ない場合がある)<br>・スポーツの得点(変化のタイミングが予測できない)<br>・ニュース記事(頻繁に変更されることが多い)<br>・コメント(頻繁に変更される)<br>・在庫レベル(短期間は一定)<br>・株価(短期間は一定で頻繁に更新される)|・キャッシュ不可と思われているが、**キャッシュ可能**<br>・CDNでキャッシュする場合、変更を検知したらインスタントパージを行い、再度オリジンサーバへアクセスするといった仕組みが必要|

- イベント駆動型コンテンツのキャッシュについて
  - イベント駆動型コンテンツの変更は、以下によって行われる。
    - ユーザが行ったアクション
    - サーバが行ったアクション
    - 管理作業
  - これらの変更は、どれもアプリケーションが認知しているので、変更を検知したら、CDNから削除し(インスタントパージ)、オリジンサーバへ再度リクエストすれば良い
    - もしインスタントパージの機能がない場合、イベント駆動型コンテンツのキャッシュは困難
...
- 参考
  ...
  - [Leveraging your CDN to cache "uncacheable" content](https://www.fastly.com/blog/leveraging-your-cdn-cache-uncacheable-content)

## 意見

- 主に画像、バンドル済JavaScriptしかキャッシュしない
  - 事故が怖い
  - Next.jsのレポートより
      - リソースの半分は画像 -> **画像がキャッシュできれば十分**
      - 最初に必要なリソースのみを読み込む
- manifest.json 等一度あげたら変わらないもの
- cdn系のサービスを選ぶときのプロバイダ選定基準
  - Fastly
    - インスタントパージ機能あり
  - **どれだけ一瞬でキャッシュを消せるかを重要視する**
- Nginx
  - パージ用のエンドポイントがある
  - (nginxのキャッシュパージ用モジュール、どれだったか調べる)

- 動的コンテンツはキャッシュしないことが多い
- Redis or Nginxのキャッシュ
  - Redisのキャッシュより2,3倍Nginxの方が早い
    - Nginxはサーバまで到達しない
  - 緻密に制御したい場合はRedis
...
|ヘッダ名|概要|値の例|備考|
|--------|-----------------------------|---------|-----------|
|`Pragma`|リソースをキャッシュするが検証が必要。<br>[draft-ietf-httpbis-cache-latest](https://httpwg.org/http-core/draft-ietf-httpbis-cache-latest.html#field.pragma)では非推奨とされている。|`no-cache`|HTTP/1.0クライアントとの後方互換用に限り使用する。それ以外の場合は`Cache-Control`ヘッダを使用|
...
|検証方法|概要|値の例|備考|
|-------|---------|--------------|--------------|
|強い検証|サーバ側とクライアント側でデータが1バイトも違わない完全一致の状態|`"xyzzy"`|HTTPは既定では強い検証を使用する|
|弱い検証|データは完全一致していないが、リソースの意味合いとしては変化しておらず同じとみなせる状態|`W/"xyzzy"`|webページなどで広告などの情報がアクセスのたびに指し変わるもののリソース的には同じ場合などに利用|
...
- 参考
  - MDN Web Docsの以下のシーケンス図は、期限切れモデル
