...
- トランザクション整合性と結果整合性
  ...
  - 使い分け
    - > ひとつの集約上でコマンドを実行するときに、他の集約のコマンドも実行するようなビジネスルールが求められるのなら、その場合は結果整合性を使うこと。
      - ドメインエキスパートは、電算化前の業務の方がよっぽど時間がかかっていたので、多少整合性を保つことに時間がかかっても許してくれる場合も意外と多い
    - > ユースケースについて考えるときは、データの整合性を保つのが誰の役割なのかに注目しよう
...
- DDDの本では、ER図に関する記述があまり出てこないため、DDDにおいて、ER図はどう考えるべきなのかを再度整理してみる。
